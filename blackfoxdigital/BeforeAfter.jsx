/* BeforeAfter — draggable comparison slider with an entrance "auto-sweep" demo.
   Props: before, after, ratio, start (0..1), beforeLabel, afterLabel,
          afterFit, afterBg, auto (run entrance sweep), autoDelay (ms), style */
function BeforeAfter({ before, after, ratio = "16 / 10", start = 0.5,
                       beforeLabel = "Before", afterLabel = "After",
                       afterFit = "cover", afterBg = "#f4f3ef", auto = false,
                       autoDelay = 500, style }) {
  const ref = React.useRef(null);
  const [pos, setPos] = React.useState(start);
  const [drag, setDrag] = React.useState(false);
  const [w, setW] = React.useState(0);
  const [touched, setTouched] = React.useState(false);   // user has interacted
  const rafRef = React.useRef(0);
  const touchedRef = React.useRef(false);

  React.useEffect(() => {
    const el = ref.current;
    if (!el || !window.ResizeObserver) return;
    const ro = new ResizeObserver(() => setW(el.getBoundingClientRect().width));
    ro.observe(el);
    setW(el.getBoundingClientRect().width);
    return () => ro.disconnect();
  }, []);

  const markTouched = () => { touchedRef.current = true; setTouched(true); cancelAnimationFrame(rafRef.current); };

  const move = React.useCallback((clientX) => {
    const el = ref.current;
    if (!el) return;
    const r = el.getBoundingClientRect();
    let p = (clientX - r.left) / r.width;
    p = Math.max(0, Math.min(1, p));
    setPos(p);
  }, []);

  React.useEffect(() => {
    if (!drag) return;
    const mv = (e) => { move(e.touches ? e.touches[0].clientX : e.clientX); };
    const up = () => setDrag(false);
    window.addEventListener("mousemove", mv);
    window.addEventListener("mouseup", up);
    window.addEventListener("touchmove", mv, { passive: true });
    window.addEventListener("touchend", up);
    return () => {
      window.removeEventListener("mousemove", mv);
      window.removeEventListener("mouseup", up);
      window.removeEventListener("touchmove", mv);
      window.removeEventListener("touchend", up);
    };
  }, [drag, move]);

  // ---- entrance auto-sweep: wipe across once to reveal the transformation ----
  React.useEffect(() => {
    if (!auto) return;
    const reduce = window.matchMedia && window.matchMedia("(prefers-reduced-motion: reduce)").matches;
    if (reduce) return;

    const el = ref.current;
    let started = false;
    const easeInOut = (t) => (t < 0.5 ? 2 * t * t : 1 - Math.pow(-2 * t + 2, 2) / 2);
    // keyframes of handle position over the sweep
    const keys = [
      { t: 0.0,  p: 0.85 },   // start: mostly showing BEFORE/raw
      { t: 0.42, p: 0.16 },   // sweep across to reveal AFTER/edited
      { t: 0.72, p: 0.62 },   // ease back
      { t: 1.0,  p: start },  // settle
    ];
    const DUR = 2100;

    const run = () => {
      if (started || touchedRef.current) return;
      started = true;
      setPos(keys[0].p);
      const t0 = performance.now() + autoDelay;
      const tick = (now) => {
        if (touchedRef.current) return;
        const elapsed = now - t0;
        if (elapsed < 0) { rafRef.current = requestAnimationFrame(tick); return; }
        const prog = Math.min(1, elapsed / DUR);
        // find segment
        let a = keys[0], b = keys[keys.length - 1];
        for (let i = 0; i < keys.length - 1; i++) {
          if (prog >= keys[i].t && prog <= keys[i + 1].t) { a = keys[i]; b = keys[i + 1]; break; }
        }
        const segT = (prog - a.t) / (b.t - a.t || 1);
        const p = a.p + (b.p - a.p) * easeInOut(Math.max(0, Math.min(1, segT)));
        setPos(p);
        if (prog < 1) rafRef.current = requestAnimationFrame(tick);
      };
      rafRef.current = requestAnimationFrame(tick);
    };

    // start when scrolled into view
    if (el && "IntersectionObserver" in window) {
      const io = new IntersectionObserver((ents) => {
        ents.forEach((e) => { if (e.isIntersecting) { run(); io.disconnect(); } });
      }, { threshold: 0.4 });
      io.observe(el);
      return () => { io.disconnect(); cancelAnimationFrame(rafRef.current); };
    } else {
      run();
      return () => cancelAnimationFrame(rafRef.current);
    }
  }, [auto, autoDelay, start]);

  const onKey = (e) => {
    if (e.key === "ArrowLeft")  { markTouched(); setPos((p) => Math.max(0, p - 0.04)); }
    if (e.key === "ArrowRight") { markTouched(); setPos((p) => Math.min(1, p + 0.04)); }
  };

  const pct = (pos * 100).toFixed(2) + "%";

  return (
    <div
      ref={ref}
      className={"ba" + (drag ? " dragging" : "") + (touched ? " touched" : "")}
      tabIndex={0}
      role="slider"
      aria-valuenow={Math.round(pos * 100)}
      aria-label="Before and after comparison"
      style={{ aspectRatio: ratio, ...style }}
      onKeyDown={onKey}
      onMouseDown={(e) => { markTouched(); setDrag(true); move(e.clientX); }}
      onTouchStart={(e) => { markTouched(); setDrag(true); move(e.touches[0].clientX); }}
    >
      {/* AFTER as base */}
      <img src={after} alt={afterLabel} style={{ objectFit: afterFit, background: afterBg }} draggable="false" />
      {/* BEFORE clipped to the left of the handle */}
      <div className="ba-after-wrap" style={{ width: pct }}>
        <img
          src={before}
          alt={beforeLabel}
          draggable="false"
          style={{ width: w ? w + "px" : "100%", objectFit: "cover" }}
        />
      </div>

      <span className="ba-tag before" style={{ opacity: pos > 0.14 ? 1 : 0, transition: "opacity .2s" }}>{beforeLabel}</span>
      <span className="ba-tag after"  style={{ opacity: pos < 0.86 ? 1 : 0, transition: "opacity .2s" }}>{afterLabel}</span>

      <div className="ba-handle" style={{ left: pct }}>
        <span className="ba-knob">
          <svg viewBox="0 0 9 14" fill="none"><path d="M8 1 2 7l6 6" stroke="currentColor" strokeWidth="1.7" strokeLinecap="round" strokeLinejoin="round"/></svg>
          <svg viewBox="0 0 9 14" fill="none"><path d="M1 1l6 6-6 6" stroke="currentColor" strokeWidth="1.7" strokeLinecap="round" strokeLinejoin="round"/></svg>
        </span>
      </div>
    </div>
  );
}

window.BeforeAfter = BeforeAfter;
